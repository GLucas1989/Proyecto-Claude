...
function LoginForm() {
  ...
  // Si ya hay sesión activa, no mostrar el login: ir directo al destino
  useEffect(() => {
  ...
  async function handleOAuth(provider: "google" | "twitch" | "discord") {
    setLoading(provider);
    setError(null);
          ...
          {sent ? (
            ...
            <>
              ...
              <button
                ...
                disabled={!!loading}
                className="flex items-center gap-3 px-4 py-3 rounded-xl border border-white/10 bg-white/[0.03] hover:border-indigo-500/40 hover:bg-indigo-500/8 text-white/60 hover:text-white/90 text-sm font-mono transition-all disabled:opacity-50"
              >
                {loading === "discord" ? <Loader2 className="h-4 w-4 animate-spin" /> : <IconDiscord className="h-4 w-4 text-indigo-400" />}
